...
def getDoodles(latitude, longitude, direction):
    ...
    sql = "Select id,path from metadata join doodles on metadata.id=doodles.metadata_id where Round(latitude,%s) = %s and Round(longitude,%s) = %s and direction > (%s - %s) and direction < (%s + %s) order by votes DESC limit 1;" % (defines.ROUNDING, defines.PRECISION % latitude, defines.ROUNDING, defines.PRECISION % longitude, direction, defines.FUDGE, direction, defines.FUDGE)
    print(sql)
    cursor.execute(sql)
    res = cursor.fetchone()
